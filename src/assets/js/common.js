...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ── 모바일 2뎁스 셀렉트 (HTML/JSP 렌더링, JS는 열고 닫기만) ── */
  function initMobileSidebarSelect() {
    document.querySelectorAll('.sidebar-mob-select').forEach(root => {
      if (root.dataset.bound === 'true') return;

      const trigger = root.querySelector('.sidebar-mob-select__trigger');
      const labelEl = root.querySelector('.sidebar-mob-select__label');
      const menu = root.querySelector('.sidebar-mob-select__menu');
      const options = [...root.querySelectorAll('.sidebar-mob-select__option[href]')];
      if (!trigger || !labelEl || !menu || !options.length) return;

      const currentPath = window.location.pathname.replace(/\/index\.html$/, '/');
      const isCurrent = (option) => {
        const href = option.getAttribute('href') || '';
        const activePrefix = option.dataset.activePrefix || '';
        let path = '';
        try {
      ...
      const activeOption =
        options.find(option => option.getAttribute('aria-current') === 'page') ||
        options.find(option => option.closest('.sidebar-mob-select__item')?.classList.contains('is-active')) ||
        options.find(isCurrent) ||
        options[0];

      options.forEach(option => {
        const item = option.closest('.sidebar-mob-select__item');

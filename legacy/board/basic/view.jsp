...
<c:if test="${not empty boardMaster.hderCn}">
	...
	<div class="siiruBoard-view">
		...
		<div class="siiruBoardBtnInfo">
			...
			<%-- 공개전환버튼. 관리자이고 비밀글사용이면 --%>
			<c:if test="${btn.openBtn eq 'Y'}">
				<button class="openBtn siiru-btn siiru-btn-warning siiru-ml5" data-action="SN" type="button"><spring:message code="button.open" /></button>
			</c:if>
			<%-- 비공개전환버튼. 관리자이고 비밀글사용이면 --%>
			<c:if test="${btn.secretBtn eq 'Y'}">
				<button class="secretBtn siiru-btn siiru-btn-danger siiru-ml5" data-action="SY" type="button"><spring:message code="button.secret" /></button>
			</c:if>
...
function boardView() {
	...
	<c:if test="${btn.openBtn eq 'Y'}">
		...
		$('.openBtn').click(function(e) {
			var formData = $('#boardSearchForm').serializeArray();
				formData.push({name:'action', value:$(this).data('action')});
				formData.push({name:'seq', value:$('#seq').val()});
			if (confirm('<spring:message code="info.open.save" />')) {
				$.post('${path.context}setBoardSecret.do', formData).done(function(data) {
					if (data.error == 'N') {
						// 페이지 새로고침
						window.location.reload(true);
					} else {
	...
	<c:if test="${btn.secretBtn eq 'Y'}">
		// 비공개 전환
		$('.secretBtn').click(function(e) {
			var formData = $('#boardSearchForm').serializeArray();
				formData.push({name:'action', value:$(this).data('action')});
				formData.push({name:'seq', value:$('#seq').val()});
			if (confirm('<spring:message code="info.secret.save" />')) {
				$.post('${path.context}setBoardSecret.do', formData).done(function(data) {
					if (data.error == 'N') {
						// 페이지 새로고침
						window.location.reload(true);
